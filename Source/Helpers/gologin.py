...
from Helpers.extensionsManager import *
...
class GoLogin(object):
    ...
    def createStdProfile(self,proxy = None):
        options = {
            "name": 'nvm iherbs',
            "os": 'win',
            "navigator": {
            ...
            'proxyEnabled': True if proxy != None else False, # Specify 'false' if not using proxy
            'proxy': {
                'mode': "none" if proxy == None else proxy["type"],
                'autoProxyRegion': 'us',
                "host": "" if proxy == None else proxy["host"],
                "port": "" if proxy == None else proxy["port"],
                "username": "" if proxy == None else proxy["port"] ,
                "password": "" if proxy == None else proxy["password"],
            },
            "webRTC": {
                "mode": "alerted",
                "enabled": True,
